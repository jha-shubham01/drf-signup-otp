      ...
      <v-col cols="4">
        <v-card ref="form" lazy-validation class="text-center">
          ...
          <v-card-text>
              ...
              <v-btn
                color="success"
                :disabled="!newUser.phoneNumber || !newUser.password1 || !newUser.password2"
                @click="register"
              >
                Register
              </v-btn>
  ...
  data: () => ({
    ...
    newUser: {
      ...
      phoneNumberRules: [
        ...
        (v) => (v && v.length <= 10) || "Phone Number must be less than 11 characters",
        (v) => (v && v.length === 10 && /[0-9]{10}/.test(v)) || "Only numbers allowed"
      ],
      passwordRules: [
